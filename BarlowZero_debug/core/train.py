...
def _train(model, target_model, replay_buffer, shared_storage, batch_storage, config, summary_writer):
    ...
    while step_count < config.training_steps + config.last_steps:
        ...
        if step_count % config.checkpoint_interval == 0:
            shared_storage.set_weights(model.get_weights())
...
def train(config, summary_writer, model_path=None):
    ...
    storage = SharedStorage(model, target_model)
    ...
    # reanalyze workers
    cpu_workers = [BatchWorker_CPU(idx, replay_buffer, storage, batch_storage, mcts_storage, config) for idx in range(config.cpu_actor)]
    workers += [cpu_worker.run() for cpu_worker in cpu_workers]
    gpu_workers = [BatchWorker_GPU(idx, replay_buffer, storage, batch_storage, mcts_storage, config) for idx in range(config.gpu_actor)]
    workers += [gpu_worker.run() for gpu_worker in gpu_workers]

    # self-play workers
    data_workers = [DataWorker(rank, replay_buffer, storage, config) for rank in range(0, config.num_actors)]
    workers += [worker.run() for worker in data_workers]
    # test workers
    workers += [_test(config, storage)]
